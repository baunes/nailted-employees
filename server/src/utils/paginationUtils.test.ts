...
describe('Paginations', () => {
  ...
  test('initializes correctly totalPages', () => {
    const p0 = PaginationUtils.getTotalPages(Pagination.basic(0, 0), 0);
    expect(p0.totalPages).toBe(0);
    expect(p0.totalItems).toBe(0);

    const p1 = PaginationUtils.getTotalPages(Pagination.basic(0, 5), 3);
    expect(p1.totalPages).toBe(1);
    expect(p1.totalItems).toBe(3);

    const p2 = PaginationUtils.getTotalPages(Pagination.basic(1, 4), 7);
    expect(p2.totalPages).toBe(2);
    expect(p2.totalItems).toBe(7);

    const p3 = PaginationUtils.getTotalPages(Pagination.basic(1, 4), 8);
